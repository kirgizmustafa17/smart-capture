...
window.SmartContentController = (function () {
  ...
  function activateMode(mode) {
    ...
    switch (mode) {
      ...
      case 'VISIBLE':
        ...
        break;
      case 'FULLPAGE':
        SmartFullPage.capture((finalDataUrl) => processCapturedImage(finalDataUrl));
        break;
      default:
  ...
  function processCapturedImage(dataUrl) {
    chrome.storage.sync.get(['autoCopy', 'autoDownload'], async (settings) => {
      if (settings.autoCopy || settings.autoDownload) {
        const blob = await SmartUtils.dataURLToBlob(dataUrl);
